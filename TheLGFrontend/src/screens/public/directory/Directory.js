...
class Directory extends Component {
  ...
  handleSearch(event) {
    Axios.get("https://www.lebarongaleana-api.com/api/directory/", {
      params: {
        ...
        return (
          <div className="results-container" key={item.id}>
            ...
            >
              ...
              <a href={`tel:${item.phone_number}`} className="loc-phone">{item.phone_number}</a>
              <div className="loc-category">{item.category}</div>
              <div className="loc-links">
                {this.props.isAuthenticated && item.businessPage && item.businessPage.status == "Live" ? (
                  <Link
                ...
                <a
                  href={
                    `https://www.google.com/maps/@${item.latitude},${item.longitude},18z`
                  }
                  className="loc-link"
                  target="blank"
                >
