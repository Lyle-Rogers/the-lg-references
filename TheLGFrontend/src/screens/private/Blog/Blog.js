...
class Blog extends Component {
  ...
  componentDidMount() {
    Axios.get("https://www.lebarongaleana-api.com/api/blog/", {
      headers: {
        Authorization: `Token ${localStorage.getItem('userTokenLG')}`,
      },
      params: {
      ...
      <div className="all-vw">
        <div className="blog">
          ...
          <div className="blog-footer">
            <button type="button" className="load-more-button" disabled={this.state.next ? false : true} onClick={this.handleNextItems}>Load More</button>
          </div>
        </div>
      </div>
    );
