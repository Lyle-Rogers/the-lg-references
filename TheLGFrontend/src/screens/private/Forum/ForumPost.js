...
const ForumPost = (props) => {
  ...
  const editPost = () => {
    Axios.put(
      `https://www.lebarongaleana-api.com/forum/${post.id}/`,
      {
        title: postTitle,
        text: postText,
      },
  ...
  const deletePost = () => {
    if (confirm("Confirm You Want To Delete Comment.")) {
      Axios.delete(`https://www.lebarongaleana-api.com/forum/${post.id}/`, {
        headers: {
          Authorization: `Token ${localStorage.getItem("userTokenLG")}`,
        }
      })
  ...
  const getPost = () => {
    Axios.get(`https://www.lebarongaleana-api.com/forum/${postId}`, {
      headers: {
        Authorization: `Token ${localStorage.getItem("userTokenLG")}`,
      },
    }).then((res) => {
  ...
  const getComments = () => {
    Axios.get(`https://www.lebarongaleana-api.com/forum/${postId}/comments/`, {
      headers: {
        Authorization: `Token ${localStorage.getItem("userTokenLG")}`,
      },
    }).then((res) => {
  ...
  const submitComment = (e) => {
    ...
    Axios.post(
      `https://www.lebarongaleana-api.com/forum/${post.id}/comments/`,
      {
        createdBy: user.id,
        post: post.id,
        text: inputText,
