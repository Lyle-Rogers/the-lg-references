...
export default class MyClassifieds extends Component {
  ...
  deleteListing(type, listingId) {
    if (window.confirm("Are you sure you want to delete this listing?")) {
      Axios.delete(`https://www.lebarongaleana-api.com/api/listing/${type}/`, {
        headers: {
  ...
  handleSearch(e) {
    Axios.get(`https://www.lebarongaleana-api.com/api/myListings/?search=${e.target.value}`, {
      headers: {
        Authorization: `Token ${localStorage.getItem("userTokenLG")}`,
      },
    })
      ...
      return this.state.items.listings.map((item) => {
        ...
        return (
          <div className="item">
            ...
            <img
              onClick={() => this.props.history.push(`/dashboard/classifieds/item/${item.id}`)}
              src={
      ...
      return this.state.items.vehicles.map((item) => {
        ...
        return (
          <div className="item">
            ...
            <img
              onClick={() => this.props.history.push(`/dashboard/classifieds/vehicles/${item.id}`)}
              src={
                thumb_nail ? thumb_nail.file : "https://via.placeholder.com/150"
              }
              alt=""
              className="item-img"
            />
            <a href={`/dashboard/classifieds/vehicles/${item.id}`} className="text">
              <div className="price">{item.price}</div>
              <div className="description">{item.title}</div>
              <div className="location">{item.location}</div>
            </a>
    ...
    return (
      <div className="classifieds">
        ...
        <div className="c-body">
          <div className="items">{this.handleVehicles()}{this.handleItems()}</div>
        </div>
      </div>
    );
  }
