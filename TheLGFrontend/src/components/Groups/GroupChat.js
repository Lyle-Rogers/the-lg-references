...
import { faArrowRight, faEllipsisH, faTrash } from "@fortawesome/free-solid-svg-icons";
...
class Groupchat extends Component {
  ...
  handleDelete(id){
    if(confirm("Confirm you want to delete message.") === true){
      ...
      Axios.post(
        `https://www.lebarongaleana-api.com/api/group_messages/${groupId}`,
        {
          post_type: "delete",
          messageId: id
        },
      ...
      if (user.id === message.creator.id) {
        return (
          <li className="self">
            ...
            <div className="info">
              <p className="options">
                <FontAwesomeIcon icon={faTrash} onClick={() => this.handleDelete(message.id)}/>
              </p>
  ...
  getMessages() {
    let groupId = this.props.groupId;
    Axios.get(`https://www.lebarongaleana-api.com/api/group_messages/${groupId}`, {
      headers: {
        Authorization: `Token ${localStorage.getItem("userTokenLG")}`,
      },
    })
